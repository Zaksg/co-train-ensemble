...
public class Loader {
     ...
     * @return
     */
    public Dataset readArff(Reader reader, int randomSeed, List<Integer> distinctValIndices,
                            int classAttIndex, double trainingSetPercentageOfDataset, FoldsInfo foldsInfo) throws Exception{
...
//            System.out.println("num of instances:  " + data.numInstances());
    ...
    private List<Fold> GenerateFoldsWithDistinctValues(ColumnInfo targetColumnInfo, int randomSeed, double trainingSetPercentage,
        ...
        for (int i=0; i<numOfFolds; i++) {
            ...
            Fold fold;
            if (isTestFold) {
                fold = new Fold(numOfClasses, FoldsInfo.foldType.Test);
            }
            else {
                fold = new Fold(numOfClasses, FoldsInfo.foldType.Train);
            }
            folds.add(fold);
        }
    ...
    private List<Fold> GenerateFolds(ColumnInfo targetColumnInfo, int randomSeed, FoldsInfo foldsInfo) throws Exception {
        Properties properties = new Properties();
        InputStream input = this.getClass().getClassLoader().getResourceAsStream("config.properties");
        properties.load(input);

        // We need to get the number of classes (we assume the target class is discrete)
        int numOfClasses = ((DiscreteColumn)targetColumnInfo.getColumn()).getNumOfPossibleValues();
        ...
        //get the number of instances for each fold type (also performs limited validation)
        HashMap<FoldsInfo.foldType, HashMap<Integer,Integer>> numOfInstancesPerFoldType =
                foldsInfo.getNumOfInstancesPerFoldPerClass(itemIndicesByClass, targetColumnInfo.getColumn().getNumOfInstances());
        ...
        //Now create the Fold objects and start filling them
        ArrayList<Fold> folds = new ArrayList<>(numOfClasses);
        for (FoldsInfo.foldType foldType : foldsInfo.getNumOfFolds().keySet()) {
            for (int i = 0; i < foldsInfo.getNumOfFolds().get(foldType); i++) {
                Fold fold = new Fold(numOfClasses, foldType);
                folds.add(fold);
            }
        }
        ...
        for (int i=0; i < targetColumnInfo.getColumn().getNumOfInstances(); i++) {
            ...
            while (!foundAssignment) {
                //We now randomly sample a fold and see whether the instance can be assigned to it. If not, sample again
                int foldIdx = rnd.nextInt(folds.size());
                if (!exploredIndices.contains(Integer.toString(foldIdx))) {
                    exploredIndices.add(Integer.toString(foldIdx));
                }
                ...
                if (exploredIndices.size() == folds.size() && !foundAssignment) {
                    throw new Exception("error in instances assignment");
                }
            }
        }
        return folds;
    ...
    private List<Fold> GenerateFolds(ColumnInfo targetColumnInfo, int randomSeed, double trainingSetPercentage) throws Exception {
        ...
        for (int i=0; i<numOfFolds; i++) {
            boolean isTestFold = designateFoldAsTestSet(numOfFolds, i, properties.getProperty("testFoldDesignation"));
            Fold fold = new Fold(numOfClasses, (isTestFold) ? FoldsInfo.foldType.Test : FoldsInfo.foldType.Train);
            folds.add(fold);
        }
